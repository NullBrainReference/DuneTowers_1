...
public class HelicopterUnitController : UnitController, IMoveToDefaultCell
{
    ...
    private HelicopterDeviceTargetController helicopterDeviceTargetController;
    public UnitController unitControllerFactory;

    private void Awake()
    {
        unitStats = GameStats.Instance.GetUnit(Unit.Helicopter, PlayerNo);
        helicopterDeviceTargetController = GetComponent<HelicopterDeviceTargetController>();
        InitUnitStats();
    }
    ...
    {
        if (CellsManager.Instance.InRangeArray(pos.x + 1, pos.y) && CellsManager.Instance.cellControllersArr[pos.x + 1, pos.y].IsFreeToMoveOver(this))
            return true;
        if (CellsManager.Instance.InRangeArray(pos.x - 1, pos.y) && CellsManager.Instance.cellControllersArr[pos.x - 1, pos.y].IsFreeToMoveOver(this))
            return true;
        if (CellsManager.Instance.InRangeArray(pos.x, pos.y + 1) && CellsManager.Instance.cellControllersArr[pos.x, pos.y + 1].IsFreeToMoveOver(this))
            return true;
        if (CellsManager.Instance.InRangeArray(pos.x, pos.y - 1) && CellsManager.Instance.cellControllersArr[pos.x, pos.y - 1].IsFreeToMoveOver(this))
            return true;
    ...
    {
        return helicopterDeviceTargetController.GetSituationResult(units, enemyUnits);
    }
