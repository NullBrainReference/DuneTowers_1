...
public class TankDeviceTargetController : MonoBehaviour
{
    ...
    internal DeviceController.SituationResult GetSituationResult(List<UnitController> units, List<UnitController> enemyUnits)
    {
        ...
        foreach (var item in enemyUnitsWithoutObstructions)
        {
            if (item.CellControllerBase != null)
            {
                ...
                enemyItems.Add(new EnemyItem(item, courseSearcher.Mas[pos.x, pos.y], pos));// courseSearcher.Mas[pos.x, pos.y]));
        ...
        allUnits.AddRange(unitsWithoutPlate);
        allUnits.AddRange(enemyUnitsWithoutPlate);

        var blocks = GetBlocks(allUnits);

        EnemyItem enemyItem = GetEnemyItem(enemyItems, blocks);
